...
def draw(canvas):
    ...
    # draw and update the rocks on screen
    process_sprite_group(rock_group, canvas)
    
    # check collision between rock and a ship
    if group_collide(rock_group, my_ship):
        lives -= 1
    
    #check collision between missile and a rock
...
def rock_spawner():
    ...
    if len(rock_group) < 12:
        ...
        if dist(rock_pos, my_ship.get_position()) <= my_ship.get_radius() + 40:
            return
        rock_vel = [random.random() * .8, random.random() * .8]
        rock_avel = random.random() * .2 - .1
        rock_group.add(Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info))

# Process sprite group function. Call update and draw for each sprite in the group
...
# collision check between a group and a sprite
...
# collision check between two groups of sprites
